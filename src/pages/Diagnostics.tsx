...
export function Diagnostics() {
  ...
  return <div className="field-page"><h1>Developer diagnostics</h1><p className="text-slate-600 mt-2">Trace requests, inspect validation and failures, and download a redacted diagnostic bundle.</p>
    ...
    <div className="field-panel flex flex-wrap gap-4 items-end"><label className="field-control">Recorder mode<select value={state?.mode ?? ''} onChange={async e => { try { await api('mode', { mode: e.target.value }); await refresh(); } catch (e) { setError((e as Error).message); } }}>
