...
export function Workbench() {
  ...
  return <div className="field-page"><div className="flex flex-wrap justify-between gap-3"><div><h1>Visual workbench</h1><p className="text-slate-600">Regional signals, multi-dimensional figures and reproducible analysis.</p></div>
    ...
    {!profile ? <p role="status" className="mt-4">Loading workbench profiles…</p> : <>
      <div className="field-panel grid md:grid-cols-2 gap-4"><label className="field-control">Dataset<select value={datasetId} onChange={e => selectDataset(e.target.value)}><option value="">Select a dataset</option>{records.map(r => <option key={r.id} value={r.id}>{r.document.name} · {r.approvalState} · {r.visibility}</option>)}</select></label>
        <label className="field-control">Saved figures and favourites<select value="" onChange={e => { const selected = saved.find(f => f.id === e.target.value); if (!selected) return;
      ...
      {record?.approvalState === 'APPROVED' && spec && <>
        <section className="field-panel"><div className="flex flex-wrap items-center justify-between gap-3"><h2>Figure builder</h2><button className="field-button" onClick={() => setMenu(!menu)} aria-expanded={menu}>Tools / context menu</button></div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-4">
            <label className="field-control">View type<select value={spec.renderer} onChange={e => setSpec({ ...spec, renderer: e.target.value, style: e.target.value === 'map' ? { ...spec.style, xScale: 'linear', yScale: 'linear' } : spec.style })}>{profile.renderers.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}</select></label>
            {Object.entries(spec.channels).filter(([key]) => key !== 'region' || spec.renderer === 'map').map(([key, value]) => <label className="field-control" key={key}>{key.toUpperCase()} channel<select value={value ?? ''} onChange={e => channel(key as any, e.target.value)}>
            ...
            <label className="field-control">Palette<select value={spec.style.palette} onChange={e => style('palette', e.target.value)}>{profile.palettes.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select></label>
            {(['xScale', 'yScale'] as const).map(key => <label className="field-control" key={key}>{key}<select value={spec.style[key]} disabled={spec.renderer === 'map'} onChange={e => style(key, e.target.value as any)}><option value="linear">Linear</option><option value="log">Log10 · excludes nonpositive values</option></select></label>)}
          </div>
          ...
          <fieldset className="mt-4 border-t pt-4"><legend className="font-medium">Geography, language, time and other column filters</legend><div className="flex flex-wrap gap-2 items-end mt-2">
            <label className="field-control">Filter column<select value={filterColumn} onChange={e => setFilterColumn(e.target.value)}><option value="">Select column</option>{columns.map(c => <option value={c.key} key={c.key}>{c.label}</option>)}</select></label>
            <label className="field-control">Operator<select value={filterOperator} onChange={e => setFilterOperator(e.target.value)}><option value="equals">Equals</option><option value="contains">Contains</option><option value="range">Range (inclusive)</option></select></label>
          ...
          {spec.channels.time && <div className="flex flex-wrap gap-3 mt-4 items-center"><label className="field-control">Time frame<select value={spec.timeValue === null ? '' : String(frames.indexOf(spec.timeValue as any))} onChange={e => setSpec({ ...spec, timeValue: e.target.value === '' ? null : frames[Number(e.target.value)] })}><option value="">All frames</option>{frames.map((f, i) => <option key={i} value={i}>{String(f)}</option>)}</select></label>
