  ...
  return <details className="field-panel"><summary className="font-semibold">Import a versioned dataset · JSON or CSV</summary>
    ...
    <div className="grid md:grid-cols-2 gap-4 mt-4"><div>
      ...
      <div className="flex flex-wrap gap-3 mt-2"><label className="field-control">Header record<input type="number" min={1} value={header} onChange={e => setHeader(Number(e.target.value))} /></label>
        <label className="field-control">Delimiter<select aria-label="Delimiter" value={separator} onChange={e => setSeparator(e.target.value)}><option value=",">Comma</option><option value=";">Semicolon</option><option value={'\t'}>Tab</option></select></label>
    ...
    {draft && <form onSubmit={e => { e.preventDefault(); try { onImport(validateDataset(draft)); } catch (e) { setError((e as Error).message); } }} className="border-t mt-4 pt-4">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
        ...
        <label className="field-control">Provider profile<select aria-label="Provider profile" value={draft.providerProfileId} onChange={e => { const p = profile.providerProfiles.find(p => p.id === e.target.value)!; setDraft({ ...draft, providerProfileId: p.id, normalization: (p.normalizations.includes('unknown') ? 'unknown' : p.normalizations[0]) as any }); }}>{profile.providerProfiles.map(p => <option value={p.id} key={p.id}>{p.label}</option>)}</select></label>
        <label className="field-control">Normalization<select aria-label="Normalization" value={draft.normalization} onChange={e => setDraft({ ...draft, normalization: e.target.value as any })}>{profile.providerProfiles.find(p => p.id === draft.providerProfileId)!.normalizations.map(v => <option key={v}>{v}</option>)}</select></label>
        <label className="field-control">What language describes<select aria-label="What language describes" value={draft.languageMeaning} onChange={e => setDraft({ ...draft, languageMeaning: e.target.value as any })}>{profile.providerProfiles.find(p => p.id === draft.providerProfileId)!.languageMeanings.map(v => <option key={v}>{v}</option>)}</select></label>
