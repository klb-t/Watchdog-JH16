...
import { axisValue, filterRows, checkFigureBindings, checkFigureProfile, type FigureSpec, type DatasetRecord, type WorkbenchProfile } from '../../shared/workbench';
...
export function FigureCanvas({ record, spec, profile, onSelect, onInspect, onMenu, svgRef }: {
  record: DatasetRecord; spec: FigureSpec; profile: WorkbenchProfile; onSelect: (id: string) => void;
  ...
  return <><svg ref={svgRef} role="group" aria-label={spec.style.title || spec.name} viewBox={`0 0 ${W} ${headerHeight + visibleFacets.length * H + footerHeight}`}
    className="w-full bg-white border border-slate-200 rounded" onContextMenu={e => { e.preventDefault(); onMenu(); }}
