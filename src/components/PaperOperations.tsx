    ...
    <section className={sectionClass}><h2 className="font-semibold">{profile?.title??'Analizy prac'}</h2><p className="text-sm">{profile?.introduction}</p>
      <fieldset disabled={busy} className="space-y-3 min-w-0">
        ...
        {kind==='assessment'?<blockquote className="border-l-2 pl-3 whitespace-pre-wrap break-words">{assessment?.body.assessment.operations[Number(operationIndex)]?.quote}</blockquote>
          :<label className="block">Dokładny cytat opisujący operację<textarea aria-label="Dokładny cytat opisujący operację" className={formClass} rows={3} value={quote} onChange={e=>setQuote(e.target.value)}/></label>}
        ...
        {activeBindings.map((b,i)=><fieldset key={b.role} className="border rounded p-3 space-y-3 min-w-0"><legend>Wejście {b.role.toUpperCase()}</legend>
          ...
          <label className="block">Co reprezentuje kolumna {b.role.toUpperCase()} i jakie ma ograniczenia?<textarea aria-label={`Co reprezentuje kolumna ${b.role.toUpperCase()} i jakie ma ograniczenia?`} className={formClass} rows={2} value={b.rationale} onChange={e=>patch(i,{rationale:e.target.value})}/></label>
        </fieldset>)}
        ...
        <label className="block">Zakres analizy i odstępstwa od pracy<textarea aria-label="Zakres analizy i odstępstwa od pracy" className={formClass} rows={3} value={scopeNote} onChange={e=>setScopeNote(e.target.value)}/></label>
