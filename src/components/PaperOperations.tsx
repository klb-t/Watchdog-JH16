  ...
  return <div className="space-y-5" data-testid="paper-operations">
    ...
    <section className={sectionClass}><h2 className="font-semibold">{profile?.title??'Analizy prac'}</h2><p className="text-sm">{profile?.introduction}</p>
      <fieldset disabled={busy} className="space-y-3 min-w-0">
        <label className="block">Praca i sposób wskazania metody<select className={formClass} value={sourceKey} onChange={e=>setSourceKey(e.target.value)}><option value="">Wybierz…</option>
        ...
        <label className="block">Operacja statystyczna<select className={formClass} value={method} disabled={kind==='assessment'} onChange={e=>setMethod(e.target.value as typeof method)}>{profile?.methods.map((m:any)=><option key={m.id} value={m.id}>{m.label}</option>)}</select></label>
        <label className="block">Zatwierdzony zbiór danych<select className={formClass} value={datasetId} onChange={e=>{setDatasetId(e.target.value);setBindings([empty('a'),empty('b')]);}}><option value="">Wybierz…</option>{datasets.map(d=><option key={d.id} value={d.id}>{d.document.name}</option>)}</select></label>
        ...
        {activeBindings.map((b,i)=><fieldset key={b.role} className="border rounded p-3 space-y-3 min-w-0"><legend>Wejście {b.role.toUpperCase()}</legend>
          <div className="grid sm:grid-cols-2 gap-3"><label>Kolumna {b.role.toUpperCase()}<select className={formClass} value={b.column} onChange={e=>patch(i,{column:e.target.value})}><option value="">Wybierz…</option>{columns.map((c:any)=><option key={c.key} value={c.key}>{c.label} · {c.unit}</option>)}</select></label>
          <label>Pochodzenie danych {b.role.toUpperCase()}<select className={formClass} value={b.origin} onChange={e=>patch(i,{origin:e.target.value as typeof b.origin,substitution:null})}>{profile?.origins.map((o:any)=><option key={o.id} value={o.id}>{o.label}</option>)}</select></label></div>
          {requirements.length>0&&<label className="block">Wymaganie z pracy {b.role.toUpperCase()}<select className={formClass} value={b.requirementId??''} onChange={e=>patch(i,{requirementId:e.target.value||null,substitution:null})}><option value="">Dodatkowe wejście / niepowiązane wymaganie</option>{requirements.map((r:any)=><option key={r.id} value={r.id}>{r.id} · {r.construct}</option>)}</select></label>}
          {b.requirementId&&<label className="block">Zapisany wariant {b.role.toUpperCase()}<select className={formClass} value={b.substitution?.id??''} onChange={e=>{const s=substitutions.find(s=>s.id===e.target.value);patch(i,{substitution:s?{id:s.id,hash:s.hash}:null,...(s?{origin:s.body.kind}:{})});}}><option value="">Bez zapisanego wariantu</option>{substitutions.filter(s=>s.body.assessmentId===assessment?.id&&s.body.requirementId===b.requirementId).map(s=><option key={s.id} value={s.id}>{s.body.kind} · {s.body.source}</option>)}</select></label>}
        ...
        <label className="block">Postępowanie z brakami<select className={formClass} value={missingPolicy} onChange={e=>setMissingPolicy(e.target.value as typeof missingPolicy)}>{profile?.missingPolicies.filter((m:any)=>allowedPolicies.includes(m.id)).map((m:any)=><option key={m.id} value={m.id}>{m.label}</option>)}</select></label>
    ...
    <section className={sectionClass}><h2 className="font-semibold">Zapisane plany i wykonania</h2>
      <label className="block">Zapisany plan<select className={formClass} disabled={busy} value={selected?.id??''} onChange={e=>e.target.value&&void act(()=>open(e.target.value))}><option value="">Wybierz…</option>{catalog?.operations.map((o:any)=><option key={o.id} value={o.id}>{o.title} · {o.method} · {o.approvalState}</option>)}</select></label>
