...
export function researchPackage(record: DatasetRecord, figure: FigureSpec, profile: WorkbenchProfile, result: any | null, geometry?: GeometryLayerRecord | null) {
  ...
  if (result) {
    ...
    if (result.paperBinding) {
      const binding = result.paperBinding;
      if (canonicalHash(binding.body) !== binding.hash || !result.methodSpec.assumptions.includes(`paper_binding_sha256=${binding.hash}`)) throw new WorkbenchError('Paper context integrity mismatch.', 409);
      json('research/paper-binding.json', binding);
    }
